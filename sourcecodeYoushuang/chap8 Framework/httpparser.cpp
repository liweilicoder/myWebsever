...
HTTP_CODE parse_requestline(char* szTemp, CHECK_STATE& checkstate)
{
    ...
    *szURL++ = '\0';
    ...
    if (strcasecmp(szMethod, "GET") == 0 ) printf( "The request method is GET\n" );
    else return BAD_REQUEST;

    szURL += strspn(szURL, " \t");
    char* szVersion = strpbrk(szURL, " \t");
    if (!szVersion ) return BAD_REQUEST;

    *szVersion++ = '\0';
    szVersion += strspn(szVersion, " \t");
    if (strcasecmp(szVersion, "HTTP/1.1") != 0) return BAD_REQUEST;
    ...
    if (!szURL || szURL[0] != '/') return BAD_REQUEST;
